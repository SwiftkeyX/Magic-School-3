...
/// Stat of a hero.
///
/// Base values live in one dictionary keyed by StatType rather than one field each, and the
/// modified values are all one call to StatModifier.Apply(). The typed properties below are
/// only there to keep call sites readable (and int-typed) - they're one-liners, so adding a
/// stat costs one line here instead of nine edits across five files.
///
...
public class Stat
{
    ...
    // ========================================== base stat getter ==========================================
    public float GetBase(StatType type) => _base[type];

    // ========================================== modify stat getter ==========================================
    public float Get(StatType type) => _statModifier.Apply(type, _base[type]);

    public int HP => Mathf.RoundToInt(Get(StatType.HP));
    public int Atk => Mathf.RoundToInt(Get(StatType.Atk));
    public int DF => Mathf.RoundToInt(Get(StatType.DF));
    public int MG => Mathf.RoundToInt(Get(StatType.MG));
    public int MR => Mathf.RoundToInt(Get(StatType.MR));
    public float AttackSpeed => Get(StatType.AttackSpeed);
    public int Range => Mathf.RoundToInt(Get(StatType.Range));
    public int StartMana => Mathf.RoundToInt(Get(StatType.StartMana));
    public int MaxMana => Mathf.RoundToInt(Get(StatType.MaxMana));
    ...
    // ========================================== setter ==========================================
    // Clamped against the MODIFIED max HP, not the base - a bonus-max-HP modifier that raises
    // the ceiling has to raise the reachable ceiling too, or the health bar (which divides by
    // the modified max) could never read full.
